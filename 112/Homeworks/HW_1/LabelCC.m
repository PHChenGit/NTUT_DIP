    ...
    elseif num ==8
        ...
        for r = 1:rows
            for c = 1:cols
                if ~visited(r, c) && img(r, c) > 0
                    labelCounter = labelCounter + 1;
                    [newVisited, newLabels] = DFS_8Connected(r, c, img, labelCounter, visited, labels);
                    visited = newVisited;
                    labels = newLabels;
                end
            end
...
function [newVisited, newLabels] = DFS_8Connected(x, y, img, labelCnt, visited, labels)
    [rows, cols] = size(img);
    ...
    if x >= 1 && x <= rows && ...
        ...
        [visited, labels] = DFS_8Connected(x, y+1, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x, y-1, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x-1, y, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x+1, y, img, labelCnt, visited, labels);

        [visited, labels] = DFS_8Connected(x-1, y+1, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x-1, y-1, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x+1, y+1, img, labelCnt, visited, labels);
        [visited, labels] = DFS_8Connected(x+1, y-1, img, labelCnt, visited, labels);
    end
